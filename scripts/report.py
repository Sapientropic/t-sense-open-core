...
def render_html(
    ...
    if is_job:
        # Default template keeps the legacy card structure while allowing public labels to be rebranded.
        labels = profile_config.labels if profile_config else _default_labels()
        return template.format(
            shared_css=shared_css,
            shared_js=shared_js,
            icon_b64=icon_b64,
            report_title=_esc(labels.report_title),
            methodology_label=_esc(labels.methodology_label),
            date=date,
