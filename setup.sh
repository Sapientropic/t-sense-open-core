#!/usr/bin/env bash
# Install T-Sense open-core dependencies
...
# --- Configure T-Sense ---
T_SENSE_CONFIG_DIR="${T_SENSE_CONFIG_DIR:-$HOME/.config/t-sense}"
T_SENSE_CONFIG="$T_SENSE_CONFIG_DIR/config.toml"

if [ ! -f "$T_SENSE_CONFIG" ]; then
    mkdir -p "$T_SENSE_CONFIG_DIR"
    cp config.example.toml "$T_SENSE_CONFIG"
    echo ""
    echo "=== Next Steps ==="
