...
Edit the T-Sense config created by setup. The default path is:

- Mac/Linux: `~/.config/t-sense/config.toml`
- Windows: `%USERPROFILE%\.config\t-sense\config.toml`
...
`~/.config/t-sense/session` by default. Set `T_SENSE_CONFIG_DIR` if you want both
config and session files in a different directory.
