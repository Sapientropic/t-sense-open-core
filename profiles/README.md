...
Use `example-market-news.md` as the public starter aligned with the README demo.
Use `example.md` if you want a job-search profile and `example-airdrop.md` for
a custom-mode airdrop profile.

## Modes

**Default mode**: if no special sections are present, T-Sense uses the built-in
candidate/profile extraction schema for backward compatibility. See `example.md`.
...
If no optional sections are present, default-mode labels and schema are used for backward compatibility.
...
- Create multiple profiles for different use cases (e.g., `market-news.md`, `airdrops.md`, `frontend-jobs.md`)
