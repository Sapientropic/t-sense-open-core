...
class ReportTests(unittest.TestCase):
    def test_build_report_renders_fixed_sections_and_program_stats(self):
        ...
        self.assertIn("# T-Sense Signal Report", result.markdown)
...
"""
    ...
    def test_invalid_llm_json_writes_debug_response_and_exits_nonzero(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            input_path = root / "scan_20260506_080000.jsonl"
            profile_path = root / "profile.md"
            output_path = root / "t-sense-signal-report.md"
            input_path.write_text(
                json.dumps(sample_messages()[0], ensure_ascii=False) + "\n",
                encoding="utf-8",
            )
...
"""
    ...
    def test_html_output_writes_markdown_and_html_from_one_extraction(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            profile_path = root / "profile.md"
            output_path = root / "t-sense-signal-report.md"
            html_output_path = root / "t-sense-signal-report.html"
            input_path.write_text(
                json.dumps(sample_messages()[0], ensure_ascii=False) + "\n",
                encoding="utf-8",
            )
        ...
        self.assertIn("# T-Sense Signal Report", markdown)
        self.assertIn("<!doctype html>", html.lower())
        self.assertIn("<title>T-Sense Signal Report", html)
        self.assertIn('<h1 class="report-title">T-Sense Signal Report</h1>', html)
